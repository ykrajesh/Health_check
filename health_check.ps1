...
## Date : 1 July 2021 
## Version : 1.3 
## This scripts check the server Avrg CPU and Memory utlization along with C and D drive drive 
...
#Import Server List 
$ServerListFile = "C:\temp\AD\PS\Servers.txt" #Change the Input path 
...
$ddrive = Get-WmiObject -Class win32_Volume -ComputerName $computername -Filter "DriveLetter = 'D:'" | Select @{Name="D Drive(GB)";Expression={"{0:N1}" -f($_.Capacity/1gb)}},@{Name="FreeSpace(GB)";Expression={"{0:N1}" -f($_.freespace/1gb)}}
...
$report = "AD-Health-Report-$da" + '.htm'  #Change the export path here 
$Outputreport | out-file C:\temp\AD\PS\$report
